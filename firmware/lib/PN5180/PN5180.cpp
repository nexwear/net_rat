 ...
 */
// --- Grewbie patch: bound the BUSY-wait loops ------------------------------
// Upstream transceiveCommand() spins on digitalRead(BUSY) with no timeout, so a
// wedged reader (e.g. after a brownout) hangs the calling task forever — which
// on the node showed up as a 60 s SensingTask stall + watchdog reboot. A 50 ms
// cap turns that into a clean failure the caller (recoverReader) can act on.
#ifndef PN5180_BUSY_TIMEOUT_MS
#define PN5180_BUSY_TIMEOUT_MS 50
#endif
static inline bool pn5180WaitBusy(uint8_t pin, int level) {
  uint32_t t0 = millis();
  while (level != digitalRead(pin)) {
    if (millis() - t0 > PN5180_BUSY_TIMEOUT_MS) return false;
  }
  return true;
}
// ---------------------------------------------------------------------------
bool PN5180::transceiveCommand(uint8_t *sendBuffer, size_t sendBufferLen, uint8_t *recvBuffer, size_t recvBufferLen) {
  ...
  // 0.
  if (!pn5180WaitBusy(PN5180_BUSY, LOW)) { digitalWrite(PN5180_NSS, HIGH); return false; } // bounded: wait until busy is low
  // 1.
  ...
  // 3.
  if (!pn5180WaitBusy(PN5180_BUSY, HIGH)) { digitalWrite(PN5180_NSS, HIGH); return false; }  // bounded: wait until BUSY is high
  // 4.
